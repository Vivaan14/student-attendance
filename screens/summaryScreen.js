  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.title}>PRESENT STUDENT LIST </Text>
        <View style={styles.presentContainer}>
          {this.state.present_students.map((student, index) => (
            <Text style={{ fontSize: 18 }}> {student.name}</Text>
          ))}
        </View>
        <Text style={styles.title}>ABSENT STUDENT LIST </Text>
...
const styles = StyleSheet.create({
  presentContainer:{justifyContent:"center",alignItems:"center",marginTop:40},
  absentContainer:{justifyContent:"center",alignItems:"center",marginTop:40},
  title:{fontSize:20,fontWeight:"bold",alignSelf:"center",marginTop:40}
}) 